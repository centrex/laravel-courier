...
`laravel-package-courier` — Template/scaffold for creating new Laravel packages in this monorepo.

Namespace: `Centrex\Courier\` (replace with actual vendor/package name)

This is the reference template. When creating a new package, copy this directory and do a find-and-replace on `Centrex`, `Courier`, `courier`, and `package-description`.
...
1. Replace all `Centrex\Courier` namespace references
2. Update `composer.json` name, description, and autoload namespace
3. Rename `Courier.php` and `CourierServiceProvider.php`
